 ...
 */
const BASE = process.env.SMOKE_API ?? 'http://127.0.0.1:8787/api/v1';
const WEB = process.env.SMOKE_WEB ?? 'http://127.0.0.1:4173';
...
record('API responds', health.status === 200 && health.body?.status === 'ok', `HTTP ${health.status}`);
...
record('Web bundle served', web.status === 200 && html.includes('<div id="root"'), `HTTP ${web.status}`);
...
// 6. Sign in
const login = await call('POST', '/auth/login', { email: EMAIL, password: PASSWORD, rememberMe: false });
record('Sign-in succeeds', login.status === 200 && login.body?.status === 'authenticated', `HTTP ${login.status}`);

const session = await call('GET', '/auth/session');
csrf = session.body?.csrfToken ?? null;
record('Session established', session.status === 200 && Boolean(csrf), session.body?.workspace?.name ?? '');
...
const consultation = list.body?.items?.find((c) => c.title === 'UAE Fire Code Review') ?? list.body?.items?.[0];
...
// 9. Metrics are exposed for scraping
const metrics = await fetch(`${BASE}/metrics`);
const body = await metrics.text();
record('Metrics exposed', metrics.status === 200 && body.includes('uxe_'), `${body.split('\n').length} lines`);
