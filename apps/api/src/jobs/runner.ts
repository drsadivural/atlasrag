...
async function runReportJob(
   ...
   * Absent means yes.
   *
   * Jobs already queued when this option shipped carry no flag, and the report they were
   * asked for was the one with the evidence in it. Defaulting the other way would quietly
   * strip the matrix out of every report in the backlog.
   */
  ...
  const generated = await deps.services.documentWorker.report({
    ...
    /*
     * Excluding evidence removes the citations, not the findings.
     *
     * Every requirement tested and its verdict stay — that is the report. What goes is the
     * block under each one that starts with the document name: the source, the version,
     * the clause locator, the page and the quoted passage. That is the difference between
     * a verdict list somebody can circulate and the full evidence pack behind it.
     */
    rows: details.evidenceRows.map((row) => ({
      requirement: row.requirement,
      result: row.result,
      finding: row.finding,
      source: includeEvidence ? row.source : '',
      version: includeEvidence ? row.version : '',
      location: includeEvidence ? row.location : '',
      page: includeEvidence ? row.page : null,
      excerpt: includeEvidence ? row.excerpt : '',
    ...
    disclosures: [
      ...
      /*
       * A report without its evidence says so, on the report.
       *
       * The findings are all still there, so nothing looks missing — which is exactly why
       * the line is needed. A reader has no other way to tell that the clause, page and
       * quotation behind every verdict were left out on purpose.
       */
      ...(includeEvidence
        ? ['Every row is traceable to the source version pinned to this consultation.']
        : [
            `This edition lists the findings without their evidence. The clause, page and quoted passage behind each of the ${details.evidenceRows.length} finding(s) are not reproduced here; generate the report again with evidence included to see them.`,
          ]),
      ...(includeEvidence && details.evidenceRows.some((r) => !r.verified)
        ? [
  ...
  const artifact = await deps.repos.artifacts.create(tenant, {
    ...
    disclosures: [],
    validation: {
      // The findings are always in the file; the citations behind them may not be.
      rows: details.evidenceRows.length,
      verifiedRows: includeEvidence ? details.evidenceRows.filter((r) => r.verified).length : 0,
