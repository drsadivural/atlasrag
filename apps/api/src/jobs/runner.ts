...
async function runReportJob(
  ...
  const generated = await deps.services.documentWorker.report({
    ...
    disclosures: [
       ...
       *
       * The alternative is a document that looks like a complete compliance report,
       * carries a decision, and gives a reader no way to tell that the findings behind it
       * were left out on purpose. The count is named so the omission is measurable.
       */
      ...(includeEvidence
        ? ['Every row is traceable to the source version pinned to this consultation.']
        : [
            `This edition omits the evidence matrix. ${details.evidenceRows.length} finding(s) were assessed and are not reproduced here; generate the report again with evidence included to see them.`,
          ]),
  ...
  const artifact = await deps.repos.artifacts.create(tenant, {
    ...
    disclosures: [],
    validation: {
      rows: includeEvidence ? details.evidenceRows.length : 0,
      verifiedRows: includeEvidence ? details.evidenceRows.filter((r) => r.verified).length : 0,
      // What the file actually contains, not what was available to put in it.
      evidenceIncluded: includeEvidence,
      findingsAssessed: details.evidenceRows.length,
    },
    status: 'ready',
  });
