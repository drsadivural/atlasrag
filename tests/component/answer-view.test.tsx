...
describe('Optimal style', () => {
  it('adds the summary, a compact evidence table and the recommended actions', () => {
  ...
  it('says so plainly when a requirement has no located evidence', () => {
    renderAnswer('optimal');
    const table = screen.getByRole('table', { name: 'Compact evidence table' });
    expect(within(table).getAllByText('No located evidence').length).toBeGreaterThan(0);
    expect(within(table).getAllByText('Needs evidence').length).toBeGreaterThan(0);
  });
